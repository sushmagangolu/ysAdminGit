...
var table = $("#datatable-buttons").DataTable({
    ...
    buttons: [{
        ...
        /*, {
            className: "btn-sm btn-primary assign_button",
            text: 'Assign Leads',
            action: function(e, dt, node, config) {
              var results = $('#newform').serialize();
              if (results == '') {
                var md = '<div id="dialog" class="modal-block"><section class="panel panel-info panel-color"><header class="panel-heading"><h2 class="panel-title">Error!</h2></header><div class="panel-body"><div class="modal-wrapper"><div class="modal-text"><p>Please select the checkboxes from the data to assign records.</p></div></div><div class="row m-t-20"><div class="col-md-12 text-center"><button id="dialogCancel" class="btn btn-primary waves-effect popup-modal-dismiss">OK</button></div></div></div></section></div>';
                $.magnificPopup.open({
                  items: {
                    src: md,
                    type: 'inline'
                ...
                $.getJSON('datalist/list.php?module=users','', function(result){
                  var md = '<div id="dialog" class="modal-block"><section class="panel panel-danger panel-color"><header class="panel-heading"><h2 class="panel-title">Assign selected leads to</h2></header><div class="panel-body"><div class="modal-wrapper"><div class="modal-text">';
                  md += '<select class="form-control u_email" name="assigned_email" id="assigned_email">';
                    $.each(result.data, function(i, v) {
                      md += '<option value="' + v.userId + '">' + v.userEmail + '</option>';
                    });
                  md += '</select>';
                  md +='</div></div><div class="row m-t-20"><div class="col-md-12 text-right"><button id="assignLeads" class="btn btn-primary waves-effect" onclick="leads.assignLeads()">Assign</button><button id="dialogCancel" class="btn btn-error waves-effect popup-modal-dismiss">Close</button></div></div></div></section></div>';
                  //var md = '<div id="dialog" class="modal-block"><section class="panel panel-danger panel-color"><header class="panel-heading"><h2 class="panel-title">Are you sure?</h2></header><div class="panel-body"><div class="modal-wrapper"><div class="modal-text"><p>Are you sure that you want to assign these leads?</p></div></div><div class="row m-t-20"><div class="col-md-12 text-right"><button id="dialogConfirm" class="btn btn-danger waves-effect waves-light" onclick="leads.assignLeads()">Assign</button> <button id="dialogCancel" class="btn btn-default waves-effect popup-modal-dismiss">Cancel</button></div></div></div></section></div>';
                  $.magnificPopup.open({
                    items: {
                      src: md,
                      type: 'inline'
    ...
    "createdRow": function(row, data, dataIndex) {
        if (data['lead_data']['assign_to'] != 0) {
            $(row).addClass('external-event bg-purple');
        }
...
$('.assign_leads').click(function() {
  ...
  $.getJSON('datalist/list.php?module=users','', function(result){
      var md = '<div id="dialog" class="modal-block"><section class="panel panel-danger panel-color"><header class="panel-heading"><h2 class="panel-title">Assign Leads</h2></header><div class="panel-body"><div class="modal-wrapper"><div class="modal-text">';
      md += 'Assign selected leads to: <select class="form-control u_email" name="assigned_email" id="assigned_email">';
        $.each(result.data, function(i, v) {
          md += '<option value="' + v.userId + '">' + v.userEmail + '</option>';
        });
      md += '</select>';
      md +='</div></div><div class="row m-t-20"><div class="col-md-12 text-right"><button id="assignLeads" class="btn btn-primary waves-effect">Assign</button><button id="dialogCancel" class="btn btn-error waves-effect popup-modal-dismiss">Close</button></div></div></div></section></div>';
      $.magnificPopup.open({
          items: {
              src: md,
              type: 'inline'
...
leads = {
    ...
    viewLead: function(id) {
        ...
        $.getJSON("inc/service.php?action=getFormAndDetails&leadId=" + id, function(result) {
            ...
            $.each(result, function(i, field) {
                ...
                switch (field['type']) {
                    case 'text':
                        form += '<input type="text" class="form-control input-sm" value="' + mappVal + '" name="' + field['name'] + '" id="' + field['name'] + '"';
                        if (field['name'] == 'campaign-name' || field['name'] == 'keyword') {
                            form += ' readonly ';
                        }
                        form += '>';
                        break;
                    case 'textarea':
                        form += '<textarea id="' + field['name'] + '" class="form-control input-sm" name="' + field['name'] + '"readonly>' + mappVal + '</textarea>';
                        if (mappVal.search("kservices.knowlarity.com") != -1) {
                            audio = '<tr><td>Audio Clip</td><td><audio src="' + mappVal + '" controls=false></p></td></tr>';
                        }
                        break;
                    case 'date':
                        form += '<input type="datetime-local" class="form-control input-sm" value="' + mappVal + '" name="' + field['name'] + '" id="' + field['name'] + '" readonly>';
                        break;
                    case 'select':
                        var options = '';
                        ...
                        form += '<select class="form-control input-sm" name="' + field['name'] + '" id="' + field['name'] + '">';
                        form += options;
                        form += '</select>';
                        break;
                }
    ...
    editLead: function() {
        ...
        var data = {
            action: 'edit_lead',
            geid: $("#leadId").val(),
            status: $("#update_lead").find("select[name='status']").val(),
            source: $("#update_lead").find("select[name='source']").val(),
        ...
        $.getJSON("inc/service.php?action=getRemarks&leadId=" + id, function(logs) {
            if (logs != 0) {
                $.each(logs, function(i, log) {
                    ...
                    if (log.remark_type == 'audio') {
                        taskList += '<p><audio src="' + log.remark + '" controls=false></p>';
                    } else {
    ...
    assignLeads: function() {
        ...
        var params = {
                action: 'assign_leads',
                userId: $('#assigned_email').val(),
                leads: myArray.join(",")
            }
            //var data = "action=assign_leads&" + results;
        $.post('inc/service.php', params, function(info) {
            table.ajax.reload(null, false);
            $.magnificPopup.close();
        });
    },
    saveLeadColumns: function() {
        $(".lead_columns").mask("Please wait...");
